...
for i = 1:3 % Για κάθε αρχικό σημείο (0,0), (-1,-1), (1,1).
    ...
    fprintf('\n--- Levenberg-Marquardt | Αρχικό Σημείο: %s ---\n', point_str);
    
    figure('Name', ['LM Trajectories - Start: ' point_str], 'NumberTitle', 'off', 'Position', [100, 500, 1200, 350]);
    ...
    for j = 1:3 % Για κάθε στρατηγική βήματος.
        ...
        % Κλήση της τροποποιημένης μεθόδου Newton.
        [x_hist, f_hist, k] = levenberg_marquardt(f, grad_f, hess_f, x0, step_type, epsilon, max_iter);
    ...
    sgtitle(['Levenberg-Marquardt: Πορείες Σύγκλισης - Εκκίνηση: ' point_str]);

    
    figure('Name', ['LM Convergence f(x) - Start: ' point_str], 'NumberTitle', 'off', 'Position', [100, 100, 1200, 350]);
    
    for j = 1:3
         step_type = step_strategies{j};
         
         [~, f_hist, ~] = levenberg_marquardt(f, grad_f, hess_f, x0, step_type, epsilon, max_iter);
    ...
    sgtitle(['Levenberg-Marquardt: Ρυθμός Σύγκλισης f(x) - Εκκίνηση: ' point_str]);
